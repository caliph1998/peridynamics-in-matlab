...
%%
%Forces used in Liu's technique (NOT APPLICABLE IN THIS CODE
%VERSION)
forceRP = zeros(TotalNumMatPoint,2);
forceRN = zeros(TotalNumMatPoint,2);
...
engineering_stress_normal = zeros(TotalNumMatPoint,2); %Peridynamic engineering normal stress at each material point
true_stress_normal = zeros(TotalNumMatPoint,2); %Peridynamic true normal stress at each material point
...
for i = 1:TotalNumMatPoint
    PDforce(i,1) = 0;
    PDforce(i,2) = 0;
    liu_stress_normal(i,1) = 0;
    liu_stress_normal(i,2) = 0;
    for j = 1:numfam(i,1)
...
%vicinity of the smaller horzion points.
        ...
        nodefam(pointfam(i,1)+j-1,3) = BondForce_y;
                %%% select a point to calculate cross-sectional force %%%  
        Cross_area_point_x = -0.15; %originally: 0.11
        Cross_area_point_y = 0.20; %originally: -0.02
        
        %%% Summing up all bond forces for cross-sectional area
        if (coord(i,1) < Cross_area_point_x)
          if (coord(cnode,1) > Cross_area_point_x)
            dforce_x_Sum = dforce_x_Sum + BondForce_x;               
          end  
        end  
            
         if (coord(i,2) < Cross_area_point_y)
          if (coord(cnode,2) > Cross_area_point_y)
            dforce_y_Sum = dforce_y_Sum + BondForce_y;               
          end  
         end  
        if (coord(cnode,1) >= coord(i,1))
            liu_stress_normal(i,1) = liu_stress_normal(i,1) + BondForce_x;
        end
        if (coord(cnode,2) >= coord(i,2))
            liu_stress_normal(i,2) = liu_stress_normal(i,2) + BondForce_y;
        end
    end
    
    liu_stress_normal(i,1) = liu_stress_normal(i,1) * dx;
    liu_stress_normal(i,2) = liu_stress_normal(i,2) * dx;
...
for i = 1:TotalNumMatPoint
    ...
    for i = 1:TotalNumMatPoint
        ...
        %%%%Stress and strain calculataion only at time = TimeInterval
        %using Using General Hooke's Law%%%
        if (tt == TimeInterval)
            
            engineering_strain(i,1) = disp(i,1) / coord(i,1);
            engineering_strain(i,2) = disp(i,2) / coord(i,2);
            true_strain(i,1) = log(1 + engineering_strain(i,1));
            true_strain(i,2) = log(1 + engineering_strain(i,2));
            
            engineering_stress_normal(i,1) = Elastic_Modulus / (1 - Possion_ratio ^ 2) * (engineering_strain(i,1) + Possion_ratio * engineering_strain(i,2));
            engineering_stress_normal(i,2) = Elastic_Modulus / (1 - Possion_ratio ^ 2) * (engineering_strain(i,2) + Possion_ratio * engineering_strain(i,1));
            
            true_stress_normal(i,1) = engineering_stress_normal(i,1)*(1 + engineering_strain(i,1));
            true_stress_normal(i,2) = engineering_stress_normal(i,2)*(1 + engineering_strain(i,2));
        end
...
Check_time(tt,1)= tt;
Steady_check_x(tt,1) = disp(555,1);
Steady_check_y(tt,1) = disp(555,2);


% Applied pressure : Cross-sectional force per unit area
Applied_pressure(tt,1) = dforce_x_Sum*thick/NumofDiv_x;
Applied_pressure(tt,2) = dforce_y_Sum*thick/NumofDiv_y;
